...
<div class="container">
  <div class="main-grid">
    ...
    <div class="sidebar">
      ...
      <div id="statusBar" class="status-bar" style="margin-top: 12px; display: none;">
        <div class="spinner" id="statusSpinner"></div>
        <div class="status-text" id="statusText">Preparing quest...</div>
      </div>

      <div id="questLog" class="quest-log" style="margin-top: 8px; display: none;">
      </div>
    </div>
    ...
    html += `
      <div class="top-card ${borderClass}">
        ...
        <div class="stat-grid">
          ...
          <div class="stat-item">
            <div class="stat-label">Holding Period</div>
...
function renderAllResults(results, filteredCount, config) {
  ...
  let html = `<div style="margin-bottom:10px;font-size:8px;color:var(--accent-cyan);">
    ${filteredCount}/${results.length} strategies passed filters
    (WR &ge; ${config.min_win_rate || 50}%, DD &le; ${config.max_drawdown || 25}%)
  </div>`;

  html += `<div class="table-scroll"><table class="results-table">
    <thead><tr>
      <th>#</th><th>Strategy</th><th>Symbol</th><th>Trades</th>
      <th>Win Rate</th><th>Max DD</th><th>Return</th>
      <th>Sharpe</th><th>P.Factor</th><th>Avg Win</th>
      <th>Avg Loss</th><th>Calmar</th><th>Expect.</th>
    </tr></thead><tbody>`;
  ...
  results.forEach((r, i) => {
    ...
    html += `<tr>
      <td>${i+1}</td>
      <td>${getSprite(r.strategy_name)} ${r.strategy_name}</td>
      <td>${r.symbol}</td>
...
fetch('/api/strategies').then(r => r.json()).then(strategies => {
  ...
  const names = {
    'rsi_mean_reversion': ['&#x1F9D9;', 'RSI Mean Reversion', 'Mage of Mean Reversion'],
    'ema_crossover': ['&#x1F9DD;', 'EMA Crossover', 'Elven Trend Follower'],
    'bollinger_squeeze': ['&#x1F9DE;', 'Bollinger Squeeze', 'Genie of Volatility'],
    'macd_histogram': ['&#x1F977;', 'MACD Histogram', 'Ninja of Momentum'],
    'stoch_rsi_bounce': ['&#x1F9DA;', 'Stochastic RSI', 'Fairy of Oscillation'],
    'triple_ema': ['&#x1F934;', 'Triple EMA Trend', 'Prince of Pullbacks'],
    'adx_rsi': ['&#x1FA96;', 'ADX + RSI Trend', 'Knight of Strength'],
    'dual_momentum': ['&#x1F3C7;', 'Dual Momentum', 'Rider of Velocity'],
    'williams_r': ['&#x1F47E;', 'Williams %R', 'Ghost of Extremes'],
    'parabolic_sar': ['&#x1F680;', 'Parabolic SAR', 'Rocket of Trends'],
    'ichimoku_breakout': ['&#x1F30A;', 'Ichimoku Cloud', 'Wave of Prophecy'],
  };
  let html = '<div style="font-size:9px;color:var(--accent-gold);margin-bottom:12px;">&#x2694;&#xFE0F; AVAILABLE HEROES &#x2694;&#xFE0F;</div>';
  strategies.forEach(key => {
    const info = names[key] || ['&#x2694;&#xFE0F;', key, ''];
    html += `<div class="strategy-item">
      <div class="sprite">${info[0]}</div>
      <div class="info">
        <div class="name">${info[1]}</div>
